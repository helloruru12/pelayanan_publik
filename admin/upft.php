...
if (isset($_POST['submit'])) {
    ...
    if ($check !== false) {
        ...
        echo "<script>alert('File bukan gambar.'); window.location.href = 'admin.php?url=profil';</script>";
        $uploadOk = 0;
    }

    // Periksa ukuran file
    if ($_FILES["foto"]["size"] > 500000) { // 500KB ukuran file maksimal
        echo "<script>alert('File Terlalu besar.'); window.location.href = 'admin.php?url=profil';</script>";
        $uploadOk = 0;
    }

    // Perbolehkan hanya format file tertentu
    if ($imageFileType != "jpg" && $imageFileType != "jpeg" && $imageFileType != "png") {
        echo "<script>alert('Maaf, hanya format JPG, JPEG, PNG yang diperbolehkan.'); window.location.href = 'admin.php?url=profil';</script>";
        $uploadOk = 0;
    }
    ...
    if ($uploadOk == 1) {
        ...
        if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
            ...
            if (mysqli_query($koneksi, $sql)) {
                ...
                echo "<script>alert('Terjadi kesalahan saat memperbarui database.'); window.location.href = 'admin.php?url=profil';</script>";
            }
        } else {
            echo "<script>alert('Maaf, terjadi kesalahan saat mengupload file Anda.'); window.location.href = 'admin.php?url=profil';</script>";
        }
    } else {
        echo "<script>alert('Maaf, file Anda tidak terupload.'); window.location.href = 'admin.php?url=profil';</script>";
    }
}
